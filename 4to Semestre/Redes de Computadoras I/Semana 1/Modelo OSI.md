...
**I. Capa de Aplicación (Capa 7):** Aquí se genera el mensaje original. Tiene protoc
	Ejemplo: como cuando un usuario interactúa con un navegador web o un cliente de correo.
**II. Capa de Presentación (Capa 6):** Se encarga de que el mensaje sea legible para el receptor. Pone la *sintaxis*, *semántica* y *codificación* de los datos.
...
**V. Capa de Red (Capa 3):** Los ***segmentos*** se vuelven ***paquetes*** (o datagramas). Aquí se añaden las direcciones IP de origen y destino para decidir qué ruta debe seguir la información para llegar a a su destino a través de la red. **Está el protocolo IP.**
**VI. Capa de Enlace de Datos (Capa 2):** Los ***paquetes*** se introducen en ***tramas**.* Se añaden las direcciones físicas (MAC ADDRESS) y una suma de comprobación ([[Métodos de identificación de tramas|checksum]]) al final de la trama para detectar si los bits se dañan durante el trayecto. Solamente hay servicios (no tiene protocolos).
